...
using namespace std;
//класс содержащий левую верхную координату данного квадрата
class Point
{
public:
    int x;
...
private:
    ...
    int64_t count;
    int best_count;
    vector <Point> current_matrix;//данная конфигурация
    vector <Point> best;//очередная лучшая конфигурация
...
public:
    ...
    void insert_square(Point point)//вставка данного квадрата
    {
    ...
    void remove_square(Point point)//удаление данного квадрата
    {
        for (int i = point.y; i < point.y + point.length; i++)
        {
            for (int j = point.x; j < point.x + point.length; j++)
    ...
    void print_table()//вывод данной таблицы
    {
    ...
    void print_best_table()//вывод лучшей таблицы
    {
    ...
    void copy_square()//копирование данной в матрицы в данную лучшую
    {
        for (int i = 0; i < length; i++)
        {
            for (int j = 0; j < length; j++)
    ...
    bool is_filled()//проверка заполненности столешницы квадратами
    {
    ...
    bool is_possible(int x, int y, int m)//возмонжость вставки квадрата данного размера в данную область
    {
    ...
    void backtracking()
    {
        ...
        insert_square(point);
        //вставка квадрата стороной ceil(n/2)
        ...
        } while (count < best_count * 5 && !(current_matrix.empty()));
    }
    ...
    void print_result(int multiply)//вывод результата работы программы
    {
        ...
        while (!(best.empty()))
        {
            ...
            best.pop_back();
            std::cout << point.x * multiply + 1 << " " << point.y * multiply + 1 << " " << point.length;
            if(!(best.empty()))
                std::cout << std::endl;
        }
    }
...
int min_primal_size(int size)//минимальный простой делитель
{
...
{
    int square_side;
    std::cout << "Please, input length of sub_matrix's side" << endl;
    std::cin >> square_side;
